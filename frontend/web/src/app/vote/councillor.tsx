...
export default function CouncillorVotePage() {
  ...
  useEffect(() => {
    axios.get('/api/candidates?position=COUNCILLOR').then(res => setCandidates(res.data));
  }, []);
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10">
      <h2 className="text-2xl font-bold mb-6">Councillor Candidates</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {candidates.map(c => (
          <div key={c.id} className="bg-white rounded shadow p-4 flex flex-col items-center">
            ...
            {c.constituency && <div className="mb-2">Constituency: {c.constituency}</div>}
            <button
